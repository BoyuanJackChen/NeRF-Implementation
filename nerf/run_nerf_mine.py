...
def create_nerf(args):
    ...
    if args.N_importance > 0:
        model_fine = init_new_nerf_model(
...
def train():
    ...
    for i in range(start, N_iters):
        ...
        # Only the dense layers have gradients. These are the layer indices of dense layers
        dense_layers = [2, 4, 6, 8, 10, 13, 15, 17, 19, 21, 22, 23]
        variables = []
        for li in dense_layers:
            print(li)
            variables += [models['model'].layers[li].kernel, models['model'].layers[li].bias,
                    models['model_fine'].layers[li].kernel, models['model_fine'].layers[li].bias]
        with tf.GradientTape() as tape:
            ...
            # rgb and target_s are (N_rand, 3); target is (H, W, 3)
            # trans = extras['raw'][..., -1]
        ...
        gi = 0
        for j in range(0, len(dense_layers)):
            model_i = dense_layers[j]
            gradient_i = gi
            learning_rate = args.lrate
            models['model'].layers[model_i].kernel = \
                tf.subtract(models['model'].layers[model_i].kernel,
                            tf.multiply(1., gradients[gradient_i]))
            models['model'].layers[model_i].bias = \
                tf.subtract(models['model'].layers[model_i].bias,
                            tf.multiply(1., gradients[gradient_i + 1]))
            models['model_fine'].layers[model_i].kernel = \
                tf.subtract(models['model_fine'].layers[model_i].kernel,
                            tf.multiply(1., gradients[gradient_i + 2]))
            models['model_fine'].layers[model_i].bias = \
                tf.subtract(models['model_fine'].layers[model_i].bias,
                            tf.multiply(1., gradients[gradient_i + 3]))
            gi += 4
